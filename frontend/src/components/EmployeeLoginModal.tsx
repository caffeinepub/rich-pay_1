...
import { authenticateEmployee, setSession } from '../lib/auth';
...
export default function EmployeeLoginModal({ onClose }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (employee) {
      setSession({ role: 'employee', userId: employee.id, username: employee.username, fullName: employee.fullName });
      router.navigate({ to: '/employee/my-accounts' });
    } else {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
      <div className="w-full max-w-sm bg-richpay-card border border-richpay-border rounded-2xl shadow-2xl shadow-black/50 overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-richpay-border bg-gradient-to-r from-richpay-primary/20 to-transparent">
          <div>
            <h2 className="text-white font-bold text-lg">Employee Login</h2>
            <p className="text-richpay-muted text-xs">Access your employee portal</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 flex flex-col gap-4">
          ...
          >
            {loading ? <><Loader2 size={16} className="animate-spin" /> Signing in...</> : 'Sign In as Employee'}
          </button>
        </form>
      </div>
    </div>
